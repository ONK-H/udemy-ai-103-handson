...
- ③のスキルセットが **AI エンリッチメント**の部分です。Text Split が文書を 300 字ごとに区切り（前のチャンクと 30 字重ねる）、Azure OpenAI Embedding が各チャンクをベクトルにします。
...
- **キーワード**は「送料」という語が入っている「返品・交換」のチャンク（返品送料の話）を1位にしました。**ベクトル**は意味の近さで「配送」のチャンクを1位にしています。**ハイブリッド＋セマンティック**は両方の結果を合わせ（RRF）、さらにセマンティックランカーで並べ直したものです。点数の尺度は方式ごとに違うので、方式をまたいで点数を比べません。
...
| 手順11で index not found | `.env` の `SEARCH_INDEX_NAME`（大文字小文字も区別）と、接続の Search 名を確認 |

## 試験の論点
- インデックス作成パイプラインは **データソース → スキルセット → インデックス → インデクサー**。スキルセットの Text Split（チャンク化）＋ Azure OpenAI Embedding（ベクトル化）が**統合ベクトル化**。
- クエリ時の質問文のベクトル化は、インデックスに定義した **vectorizer** が行う（アプリ側で埋め込みを呼ばなくてよい）。
