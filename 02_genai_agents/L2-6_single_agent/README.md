...
| ファイル | 役割 |
|---|---|
| `main.py` | エージェントの作成・会話（複数ターン）・ツール呼び出しの表示・削除（`finally`） |
| `.env.sample` | 環境変数の雛形 |
...
python main.py
```
エージェントを作り、同じ会話で2問続けて聞き、最後にエージェントを削除します。
```text
...
### 8. 後片付けを確かめる
手順6・7の最後の行 `エージェントを削除しました（残り: 0 件）` が、後片付けの確認です。`main.py` は `finally` で作ったエージェントの版を削除し、同じ名前のエージェントが一覧に残っていないかを数えて表示します。途中でエラーが出ても `finally` は必ず走るので、エージェントがプロジェクトに溜まりません。
...
## 後片付け
- エージェントは実行のたびに `finally` で削除されます（手順8）。削除が必要なリソースはありません。
