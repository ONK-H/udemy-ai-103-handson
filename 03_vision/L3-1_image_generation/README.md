...
> 画像生成モデルは講座共通のリージョン（japaneast）では使えないため、**このレッスン専用のリソースグループを対応リージョン（eastus2）に作ります**。次の動画生成のレッスン（L3-2）でも同じリソースを使うので、後片付けは L3-2 のあとで行います。
...
- `mask.png`：周りが白（不透明）、中央が透明（エディターでは市松模様）。**透明の部分が「描き直してよい範囲」**です。
...
### 12. 後片付け（L3-2 のあとで）
**次の L3-2（動画生成）でも同じリソースを使うので、L3-2 を続けて行う場合はここでは消しません。** L3-2 をやらない場合や、L3-2 を終えたら、次のコマンドで消します。
```powershell
$RG
