...
>
> Codespaces にはマイクもスピーカーもありません。そこで、**入力の音声は TTS で作ったファイル（`input.wav`）**を使い、**出力の音声はファイル（`output.wav`）**に書き出します。音が出ているかは、ファイルの長さ・サイズと、出力をもう一度 STT にかけた結果で確かめます（Codespaces のエディターで wav を開くと再生もできます）。
